...
def ImportData(inputs):
    ...
    if exists(inputs['Grid_path']):
        ...
        #Append everython
        isochroneIndex = [index,array(mh_list),array(age_list)]
    #If not, interact
    else:
...
def ImportIsochrone(clusterCMD,inputs,pop_params,isochroneIndex):
    ...
    raw_mh,raw_age,d,extpar = pop_params
    if raw_mh < min(mh_list):
         print('Chosen metallicity is smaller than the upper limit of the grid!')  
    elif raw_mh > max(mh_list):
        print('Chosen metallicity is larger than the upper limit of the grid!')  
    if raw_age < min(age_list):
        print('Chosen age is smaller than the lower limit of the grid')
    if raw_age > max(age_list):
        print('Chosen age is larger than the upper limit of the grid!')
    mh = MCMCsampling.Auxiliary.Round_to_Reference(raw_mh,mh_list)
    logage = MCMCsampling.Auxiliary.Round_to_Reference(raw_age,age_list)
    #Read file
    iso = read_csv('{}/{}.dat'.format(inputs['Grid_path'],isoIndex[mh][logage]),sep=',')
...
def SyntheticPopulation(clusterCMD,ClusterGrid,inputs,isochroneIndex,pop_params):
    ...
    isoIndex, mh_list,age_list = isochroneIndex
    #Get population parameters
    raw_mh,raw_age,d,extpar,binf = pop_params
    mh = MCMCsampling.Auxiliary.Round_to_Reference(raw_mh,mh_list)
...
def SaveAll(inputs,figures,clusterCMD,ClusterGrid,WeightsGrid):
    ...
    print('\n')
    print('Now you can perform the MCMC sampling by running:')
    print('\t python RunMCMCsampling.py {}'.format(inputs['Project_name']))
