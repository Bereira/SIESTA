...
def MaginalPosterior(samplesIn,inputs,labels,bins,quantiles_min,quantiles_max,title,age_plot,dist_plot,obs_label,synt_label):
    ...
    #
    #Indexes
    indexes = ['Metallicity','Age','Distance','ExtinctionPar','BinFraction']
    #Get priors
    priors = MCMCsampling.MCMCsupport.DefinePriors(inputs['Priors'])
    #
    # START FIGURE
    #
    #Create figure
    ...
    for i in range(0,sampleNum):
        ...
        elif (i == 1)&(age_plot=='Myr'):
            yp = exp([ priors[i](log10(xxx)+6, inputs['Priors'][indexes[i]]['Parameters'] ) for xxx in xx])
        elif (i==2)&(dist_plot=='modulus'):
            yp = exp([ priors[i](10**( (xxx + 5)/5 )/1000, inputs['Priors'][indexes[i]]['Parameters']) for xxx in xx])
        else:
            yp = exp([ priors[i](xxx, inputs['Priors'][indexes[i]]['Parameters'])  for xxx in xx])
        ax[i][i].plot(yp/max(yp)*max(counts),xx,c='k',ls=':')
        #Details
    ...
    #   logAge
    if age_plot == 'log':
        raw_logAge = fit_value[1]
    elif age_plot == 'Gyr':
        raw_logAge =  log10(fit_value[1])+9 
    elif age_plot == 'Myr':
        raw_logAge = log10(fit_value[1])+6 
    if raw_logAge < min(age_list):
        print('Fitted age is smaller than the lower limit of the grid')
    if raw_logAge > max(age_list):
        print('Fitted age is larger than the upper limit of the grid!')
    else:
        logAge = MCMCsampling.Auxiliary.Round_to_Reference(raw_logAge,age_list)
    #   Distance
    if dist_plot == 'modulus':
